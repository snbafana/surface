...
| --- | --- | --- | --- |
| Integration Hub | Browserbase, integrations.sh, Coast, Cued, GitHub, and Steipete-style local tools all create useful agent/workflow context, but they must not become a second plugin registry. | Implemented as `integrationhub`: read-only executable/env readiness, source links, and copyable commands. | Must stay status/copy/open only; no credential storage, network discovery daemon, generic command launcher, or registry replacement. |
| Browserbase Session Cards | Browserbase observability gives session status, live view, video recordings, network/console logs, and replay/debug handoff links. | Read cached/fixture session JSON and show status, duration, recording/debug URL, and copy/open actions. | Must not create sessions, run `browse`, fetch logs, store API keys, or embed browser control. |
| Integration Catalog Finder | integrations.sh exposes a public search/detect/discover API across MCP, OpenAPI, GraphQL, and CLI surfaces. | Cache-backed search cards for services relevant to current plugin design, with source/open/copy actions. | Must not become a network crawler, credential catalog, or plugin registry. |
| Agent Toolbelt Cards | Steipete-style tools such as Peekaboo, CodexBar, RepoBar, mcporter, oracle, and cookie utilities show compact local-tool status patterns. | Show installed/missing status and copyable commands for explicitly selected local tools. | Must not become a generic command launcher or auto-installer. |
